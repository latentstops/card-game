( () => {
    const toAppendedChildTo = appendChild;
    const toAppendChildToDocumentBody = toAppendedChildTo( document.body );
    const elementsParams = [ { id: 'c3d' },{ id: 'c2d' } ];

    elementsParams.map( toCanvasElementAppendedToDocumentBody );

    function toCanvasElementAppendedToDocumentBody( elementParam ){
        return toAppendChildToDocumentBody( toCanvasElement( elementParam ) );
    }

    function toCanvasElement( params ){
        return toElement( 'canvas', params );
    }

    function toElement( name, params ){
        const element = document.createElement( name );

        assign( element, params );

        return element;
    }

    function appendChild( parent ){
        return ( ...args ) => parent.appendChild( ...args );
    }
...
( () => {
    ...
    (function() {
        ...
        let useWebGL = true; //params.searchParams.get("webgl") !== '0';
